...
/*Последнее редактирование 14.05.2023*/ <br> 
...
Подключившись через программу Wireshark в один из каналов связи, при помощи команды ping выполняется проверка работоспособности компьютеров в сети. Изначально проходит запрос (показан розовым цветом), через некоторый промежуток (показан белым цветом) приходит ответ (показан розовым цветом). Ответ и запрос содержат информацию с ip-адресами. Через данную программу можно наблюдать главный недостаток Telnet - передачу данных без шифрования (в открытом виде).
