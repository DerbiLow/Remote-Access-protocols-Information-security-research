...
/*Последнее редактирование 21.05.2023*/ <br> 
...
Назначение TELNET протокола - двунаправленное восьмибитное взаимодействие терминального устройства и терминал-ориентированнного процесса. Для взаимодействия используются два типа восьмибитных команд. Для передачи данных старший бит представлен 0, семь младших - комбинация ASCII. Для передачи команд старший бит представлен 1, семь младших - комбинация ASCII. Эти комбинации передаются по одному и тому же каналу связи, для разделения комбинаций используют “разделяющий символ”.  Основная цель использования - организация взаимодействия "терминал-терминал" (сеанс связи), а также "процесс-процесс" (процесс вычисления). TELNET состоит из: telnet-интерфейса пользователя, telnetd-процесса, TELNET-протокола. Основными реализациями протокола Telnet являются: Клиенты - telnet.exe (Windows), telnet (Unix). Серверы - telnetd, MS Telnet. В основе данного протокола лежит концепции сетевого виртуального терминала - механизма абстрагирования от специфики ввода/вывода различных аппаратных и программных платформ. Сессия взаимодействия нескольких компьютеров подразумевает выделение клиентской и серверную сторон, протокол является симметричным и обе стороны взаимодействую друг с другом через сетевой виртуальный терминал, основные данные, которыми обмениваются стороны - прикладные (данные взаимодействия сервера и пользователя) и опции протокола telnet (описывают предпочтения сторон (те действия, которые стороны хотят сделать)). Прикладные данные могут передаваться в явном виде (последовательность байтов на ASCII) или в виде двоичного кода.
Telnet поддерживает 4 основных режима работы:
1. Полудуплексный (возможность передавать и принимать данные есть у обоих абонентов, но только поочерёдно) - сетевой виртуальный терминал - специальное полудуплексное устройство, которое требует выполнения команды "GO AHEAD, GA" от сервера перед тем, как принять запрос от пользователя. Запрос (ввод пользователя) пользователя к серверу отображается только при помощи полных строк.
...
Подключившись через программу Wireshark в один из каналов связи, при помощи команды telnet можно отследить сетевой трафик, передаваемый этим протоколом. При попытке открыть пакет данных, внутри него можно увидеть логин и пароль, передаваемые в открытом виде без какого-либо шифрования.
	
![image](https://github.com/DerbiLow/Remote-Access-protocols-Information-security-research/assets/126500303/cb98338d-d162-419d-99ee-52cac0cd2b4b)<br>
Рисунок 25 - Данные передаваемые по каналу связи <br>
	
	
![image](https://github.com/DerbiLow/Remote-Access-protocols-Information-security-research/assets/126500303/b53109c2-c031-4dcc-9624-99d44753f29c)<br>
Рисунок 26 - Незащищённый логин и пароль<br>

     7.  Разбор способов несанкционированного доступа в сеть с удаленным администрированием, а также способов защиты от данного типа взлома
                  
Способы несанкционированного доступа при помощи удалённого администрирования можно разбить на три основные группы: <br>

1. Невнимательность при использовании программ удалённого администрирования. <br>
2. Невнимательность при использовании сети интернет. <br>
3. Использование небезопасных сетей и протоколов удалённого администрирования. <br>
...
В качестве примера такой программы был найден Telegram бот, который может удалённо управлять компьютером. Файл с командами, подключением к серверу встраивается в автозагрузку компьютера. [20]
Этот Telegram бот может включать, выключать компьютер, получать скриншот рабочего экрана. Функционал бота можно легко расширить, добавив новые команды.<br>
	
![image](https://github.com/DerbiLow/Remote-Access-protocols-Information-security-research/assets/126500303/4663913d-ae6c-4c78-af3a-a019afa26d8e)<br>
Рисунок 27 - Удалённое управление компьютером с Telegram бота<br>
...
Один из самых распространённых методов перехвата трафика, является MITM (Man in the middle) - человек посередине. Это способ перехвата и изменения трафика, при котором злоумышленник находится внутри канала связи и пропускает весь поток данных через свою машину.

Чтобы попасть в канал связи, злоумышленник анализирует сетевой трафик и ищет в нем уязвимости. В данном случае две рабочие машины хотят установить взаимодействие по сетевому протоколу. Компьютер (А), который будет удалённо управлять другим, отправляет запрос через незащищённую сеть. Компьютер (B), который даёт возможность управлять своим интерфейсом, отправляет ответ через незащищённую сеть. Но ни запрос, ни ответ не дойдут через открытый канал в исходном виде, так как компьютер злоумышленника (С) поменяет данные, отправив на каждый из компьютеров ложный запрос и ответ. Таким образом, он окажется посередине между двумя компьютерами. Будет отслеживать трафик. Если А и B используют для удалённого управления протокол, который не шифрует данные, то злоумышленник сможет легко их просматривать. [21] <br>

![image](https://github.com/DerbiLow/Remote-Access-protocols-Information-security-research/assets/126500303/ed52a25e-4dff-40a0-844f-dfd6319bfaf1)<br>
...
По результатам исследования можно сделать следующие выводы: наибольшей скоростью передачи данных, защищенностью и популярностью обладают протоколы, которые совершенствуются и по сей день, а также протоколы, которые имеют возможность подключаться к множеству различных операционных систем. Протоколы с низким уровнем защиты можно использовать, но только в хорошо защищённых сетях, либо подключая к ним криптографические протоколы SSL, TLS. “Старые” сетевые протоколы используются и сегодня, однако перечень используемых устройств ограничен микропроцессорами, сетевыми устройствами. <br>

В ходе выполнения курсового проекта было исследованы и изучены: <br>
1.Информационная безопасность протоколов удалённого администрирования. <br>
2.Исследованы протоколы, которые активно используются сегодня, а также протоколы, которые по каким-либо причинам выведены из эксплуатации. <br>
3.Исследовано сетевое оборудование, на основе которого работают протоколы<br>
4.Способы получения удалённого доступа злоумышленником, а также способы защиты от данного типа взлома.<br>
5.На примере существующей программы, разобран функционал удалённого администрирования.<br>
6.OSI, уровни этой системы, протоколы, работающие на каждом уровне <br>
7.Способы подключения протокола в сети<br>
...
1. <strong>Positive technologies</strong> официальный сайт. URL - https://www.ptsecurity.com/ru-ru/research/analytics/cybersecurity-threatscape-2022-q4/?utm_source=pt&utm_medium=cpm&utm_campaign=analytic-2023-02-17&utm_content=main-slider   (Дата обращения 15.03.2023). - Текст электронный <br>
2. <strong>AweSun</strong> официальный сайт. URL - https://sun.aweray.com/ru/ (Дата обращения 15.03.2023). - Текст электронный<br>
3. <strong>teacherbox</strong> официальный сайт. URL - https://teacherbox.ru/arhiv/pm02/lesson2/1-2-19-setevyie-protokolyi-udalennogo-upravleniya/.html   (Дата обращения 20.03.2023). - Текст электронный<br>
4. <strong>infourok.ru</strong> официальный сайт. URL - https://infourok.ru/nastrojka-protokolov-udalennogo-dostupa-5444335.html  (Дата обращения 22.03.2023). - Текст электронный<br>
5. <strong>youtube.com</strong> официальный сайт. URL - https://www.youtube.com/@merionacademy  (Дата обращения 17.03.2023). - Видео формат<br>
6. <strong>wiki.merionet.ru</strong> официальный сайт. URL - https://wiki.merionet.ru/seti/27/marshrutizator-kommutator-hab-chto-eto-i-v-chem-raznica/  (Дата обращения 18.03.2023). - Текст электронныйv
7. <strong>ru.wikipedia.org</strong> официальный сайт. URL - https://ru.wikipedia.org/wiki/Протокол_передачи_данных (Дата обращения 22.03.2023). - Текст электронный<br>
8. <strong>ru.wikipedia.org</strong> официальный сайт. URL - https://ru.wikipedia.org/wiki/Алгоритм_Дейкстры   (Дата обращения 20.03.2023). - Текст электронный<br>
9. <strong>github.com</strong> официальный сайт. URL - https://github.com/DerbiLow/Kraskal-s-algorithm   (Дата обращения 20.03.2023). - Текст электронный<br>
10. <strong>securitylab.ru</strong> официальный сайт. URL - https://www.securitylab.ru/analytics/533599.php?clear_cache=Y   (Дата обращения 17.03.2023). - Текст электронный<br>
11. <strong>wikipedia.org</strong> официальный сайт. URL - https://ru.wikipedia.org/wiki/Сервер_(программное_обеспечение)  (Дата обращения 20.03.2023). - Текст электронный<br>
12. <strong>translated.turbopages.orgо</strong> официальный сайт. URL - https://translated.turbopages.org/proxy_u/en-ru.ru.5d3f6415-641c943a-2d94ef89-74722d776562/https/en.wikipedia.org/wiki/Apple_Remote_Desktop  (Дата обращения 24.03.2023). - Текст электронный<br>
13. <strong>youtube.com</strong> официальный сайт. URL - https://www.youtube.com/watch?v=Jx-1oHhVlsE   (Дата обращения 25.03.2023). - Видео формат<br>
14. <strong>ru.wikipedia.org</strong> официальный сайт. URL - https://ru.wikipedia.org/wiki/TLS (Дата обращения 25.03.2023). - Текст электронный<br>
15. <strong>ru.wikipedia.org</strong> официальный сайт. URL - https://ru.wikipedia.org/wiki/Virtual_Network_Computing  (Дата обращения 25.03.2023). - Текст электронный<br>
16. <strong>ru.wikipedia.org</strong> официальный сайт. URL -  https://ru.wikipedia.org/wiki/Протокол_RFB  (Дата обращения 25.03.2023). - Текст электронный<br>
17. <strong>ru.wikipedia.org</strong> официальный сайт. URL - https://ru.wikipedia.org/wiki/Remote_Desktop_Protocol (Дата обращения 25.03.2023). - Текст электронный<br>
18. <strong>ru.wikipedia.org</strong> официальный сайт. URL - https://ru.wikipedia.org/wiki/SLIP  (Дата обращения 25.03.2023). - Текст электронный<br>
19. <strong>ru.wikipedia.org</strong> официальный сайт. URL - https://ru.wikipedia.org/wiki/SSL  (Дата обращения 25.03.2023). - Текст электронный<br>
20. <strong>youtube.com</strong> официальный сайт. URL - https://www.youtube.com/watch?v=skRHdt94NWc (Дата обращения 21.05.2023). - Видео формат <br>
21. <strong>youtube.com</strong> официальный сайт. URL - https://www.youtube.com/watch?v=a-S_Sg2gyXo (Дата обращения 21.05.2023). - Видео формат
